...
describe('P7.2-A · Экстремальные цены', () => {
  ...
  it('USA $100K → тройной overflow', () => {
    ...
    expect(r.breakdown.fixedCosts).toBe(1_270_000);
  });
  ...
  it('Корея 10M₩ → дешёвое авто (~536K₽ + 90K + 600K)', () => {
    ...
    const r = calculate(car, RATES, EUR_RATE);
    // ETT-based: expect reasonable range
    expect(r.totalRUB).toBeGreaterThan(1_100_000);
    expect(r.totalRUB).toBeLessThan(1_600_000);
  });
  ...
  it('ОАЭ 300K AED → дорогое авто, большой overflow', () => {
    ...
    const r = calculate(car, RATES, EUR_RATE);
    // priceUSD = 300000/3.67 + 3200 ≈ 84,970
    // overflow: 84970 > 50K → fix = 560K + ceil(34970/10K)*100K = 560K + 400K = 960K₽
...
describe('P7.2-G · Все комбинации 3-5/5+ лет × направление', () => {
  ...
  it('USA 3-5 лет → РБ', () => {
    ...
    expect(r.breakdown.ageCategory).toBe('3to5');
    expect(r.breakdown.destination).toBe('BY');
    expect(r.breakdown.fixedCosts).toBe(520_000); // BY ≤$20K
  });
...
describe('P7.2-H · Гибриды', () => {
  ...
  it('Гибрид считается как ДВС (ETT)', () => {
    ...
    const rPetrol = calculate(carPetrol, RATES, EUR_RATE);
    const rHybrid = calculate(carHybrid, RATES, EUR_RATE);
...
describe('P7.2-J · Ошибки и невалидные данные', () => {
  ...
  it('China 5+ лет с eurRate → работает', () => {
    const car: CarInput = {
      country: 'China', destination: 'RU', price: 100_000, currency: 'CNY',
      year: 2019, engineType: 'petrol', engineCC: 2000, horsePower: 150,
    };
    const r5 = calculate(car, RATES, EUR_RATE);
    expect(r5.totalRUB).toBeGreaterThan(0);
  });
  ...
  it('Неизвестная страна → ошибка', () => {
    ...
    expect(() => calculate(car, RATES, EUR_RATE)).toThrow('Неизвестная страна');
  });
  ...
  it('ОАЭ 3-5 лет → ошибка (только новые)', () => {
    ...
    expect(() => calculate(car, RATES, EUR_RATE)).toThrow('только новые');
  });

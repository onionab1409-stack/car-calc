...
/** Ручной расчёт ЕТТ ЕАЭС до 3 лет: MAX(% от стоимости, мин EUR/см³) */
...
function manualUSA(lot: number, dest: 'RU' | 'BY', engineCC?: number): number {
  ...
  if (dest === 'RU') {
    ...
    customs = manualETTUnder3(customsBaseRUB, engineCC ?? 0) / RATES.USDT_RUB;
  } else {
...
function manualKorea(priceKRW: number, dest: 'RU' | 'BY', engineCC?: number): number {
  ...
  if (dest === 'RU') {
    const ettRUB = manualETTUnder3(priceRUB, engineCC ?? 0);
...
function manualUAE(priceAED: number, dest: 'RU' | 'BY', engineCC?: number): number {
  ...
  if (dest === 'RU') {
    const ettRUB = manualETTUnder3(baseRUB, engineCC ?? 0);

...
describe('calculate — 8 эталонных расчётов (≤160лс, без доплаты утильсбора)', () => {
  ...
  it('#2 🇺🇸 USA → 🇷🇺 РФ: $25K, 150лс → ~3,972,000₽', () => {
    ...
    const r = calculate(car, TEST_RATES);
    expect(r.totalRUB).toBeGreaterThan(3_972_000 * 0.995);
    expect(r.totalRUB).toBeLessThan(3_972_000 * 1.005);
    expect(r.breakdown.utilSbor).toBe(0);
  });
  ...
  it('#3 🇰🇷 Корея → 🇷🇺 РФ: 35M₩, 150лс → ~3,469,000₽', () => {
    ...
    const r = calculate(car, TEST_RATES);
    expect(r.totalRUB).toBeGreaterThan(3_469_000 * 0.995);
    expect(r.totalRUB).toBeLessThan(3_469_000 * 1.005);
    expect(r.breakdown.utilSbor).toBe(0);
  });
  ...
  it('#7 🇨🇳 Китай → 🇷🇺 РФ: 180K¥, 150лс → ~3,838,000₽', () => {
    ...
    const r = calculate(car, TEST_RATES);
    expect(r.totalRUB).toBeGreaterThan(3_838_000 * 0.995);
    expect(r.totalRUB).toBeLessThan(3_838_000 * 1.005);
    expect(r.breakdown.utilSbor).toBe(0);
  });
...
describe('calculate — с доплатой утильсбора (>160лс)', () => {
  it('USA→РФ $25K, 200лс, 2.0L → базовый + 949,400₽ утильсбор', () => {
    ...
    // Базовый расчёт ~3,972K (как #2 но horsePower не влияет на формулу)
    // + 949,400 → ~4,921K
  ...
  it('Корея→РФ 35M₩, 180лс, 1.5L → базовый + 896,600₽', () => {
    ...
    expect(r.totalRUB).toBeGreaterThan(3_469_000 + 896_600 - 20_000);
  });
...
describe('calculateQuick — роутер', () => {
  it('USA→РБ $15K совпадает с полным (до 3 лет, ≤160лс)', () => {
    ...
    const full = calculate(car, TEST_RATES);
    expect(quick).toBe(full.totalRUB);
  });

  it('Korea→РФ 35M₩ совпадает с полным (до 3 лет, ≤160лс)', () => {
    const car: CarInput = {
      country: 'Korea', destination: 'RU', price: 35_000_000, currency: 'KRW',
      year: 2024, engineType: 'petrol', engineCC: 2000, horsePower: 150,
    };
...
describe('calculate — ошибки', () => {
  ...
  it('ОАЭ 3–5 лет → ошибка', () => {
    ...
    expect(() => calculate(car, TEST_RATES)).toThrow('только новые авто');
  });
});
